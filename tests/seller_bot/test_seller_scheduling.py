"""
Integration tests for seller bot calendar scheduling.
...
from bots.seller_bot.jorge_seller_bot import JorgeSellerBot, SellerQualificationState, SellerStatus
...
def mock_ghl():
    ...
    client.create_appointment = AsyncMock(
        return_value={"success": True, "data": {"id": "appt-42"}}
    )
    return client
...
def _hot_state(contact_id: str = "contact-1") -> SellerQualificationState:
...
def _warm_state(contact_id: str = "contact-1") -> SellerQualificationState:
    # WARM requires: questions_answered>=4 + price in range + motivation + urgency != "low"
...
@pytest.mark.asyncio
async def test_hot_seller_gets_slot_offer(bot, mock_ghl):
    state = _hot_state()
    ...
    # Patch cache + response generation
    bot.cache = MagicMock()
...
@pytest.mark.asyncio
async def test_cold_seller_gets_no_scheduling(bot):
    ...
    with (
        ...
        result = await bot.process_seller_message("contact-cold", "loc-123", "hello")
...
@pytest.mark.asyncio
async def test_slot_reply_books_appointment(bot, mock_ghl):
    state = _hot_state()
    state.scheduling_offered = True
    bot.calendar_service._pending_slots["contact-1"] = [
        {"start": "2026-03-01T17:00:00Z", "end": "2026-03-01T17:30:00Z"},
    ]
    ...
    with (
        ...
        result = await bot.process_seller_message("contact-1", "loc-123", "1")

    assert "booked" in result.response_message.lower()
    assert state.appointment_booked is True
    assert state.appointment_id == "appt-42"


@pytest.mark.asyncio
async def test_booking_failure_gives_retry_message(bot, mock_ghl):
    state = _hot_state()
    state.scheduling_offered = True
    bot.calendar_service._pending_slots["contact-1"] = [
        {"start": "2026-03-01T17:00:00Z", "end": "2026-03-01T17:30:00Z"},
...
@pytest.mark.asyncio
async def test_scheduling_offered_only_once(bot, mock_ghl):
    state = _hot_state()
    ...
    with (
        ...
        await bot.process_seller_message("contact-1", "loc-123", "something else")
