...
async def validate_bot_creation():
    ...
    try:
        ...
        assert hasattr(seller_bot, '_states')
        print_success("Bot has state management")
...
async def validate_q1_q4_flow(seller_bot):
    ...
    try:
        ...
        assert r1.questions_answered == 0
...
async def validate_analytics(seller_bot):
    ...
    try:
        ...
        seller_bot._states["test_analytics"] = state
...
async def validate_concurrent_handling(seller_bot):
    ...
    try:
        ...
        assert "seller_A" in seller_bot._states
        assert "seller_B" in seller_bot._states
        assert "seller_C" in seller_bot._states
        print_success("Each conversation has independent state")
